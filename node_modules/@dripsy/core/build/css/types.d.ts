...
declare type AliasToScale<ScaleOrAlias extends keyof DripsyThemeWithoutIgnoredKeys | StyleableSxProperties> = ScaleOrAlias extends keyof Aliases ? Aliases[ScaleOrAlias] extends keyof Scales ? Scales[Aliases[ScaleOrAlias]] extends keyof DripsyThemeWithoutIgnoredKeys ? Scales[Aliases[ScaleOrAlias]] : ScaleOrAlias : ScaleOrAlias : ScaleOrAlias;
export declare type MaybeTokenizedValue<Key extends StyleableSxProperties, Scale extends keyof DripsyThemeWithoutIgnoredKeys | StyleableSxProperties = Key extends keyof Scales ? Scales[Key] extends keyof DripsyThemeWithoutIgnoredKeys ? Scales[Key] : Key : Key, AliasedScale extends Scale = AliasToScale<Scale>> = AliasedScale extends ScaleValue ? Tokenize<DripsyThemeWithoutIgnoredKeys[AliasedScale], true, false> : TokenizedTheme;
export declare type ValueOrThemeFactoryValue<T> = T | ((theme: DripsyFinalTheme) => T);
...
export declare type StyleableSxProperties = Exclude<keyof ThemeUICSSProperties, ShadowStyleKeys | 'variant'> | keyof ViewStyle | keyof TextStyle;
...
declare type NotFunction<T> = T extends (...args: any[]) => any ? never : T;
declare type SxStyles = {
    [key in StyleableSxProperties]?: (OnlyAllowThemeValueForKey<key> extends true ? ResponsiveValue<MaybeTokenizedValue<key>> : ResponsiveValue<MaybeTokenizedValue<key>> | NotFunction<NativeOrThemeUiStyle<key> & {}>) | ValueOrThemeFactoryValue<NotFunction<NativeOrThemeUiStyle<key> & {}> | ResponsiveValue<MaybeTokenizedValue<key>>> | null;
};
